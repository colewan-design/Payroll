...
<head>
  ...
  <title>CBOO | Employee Leave</title>
            ...
            <header class="py-3">
              <div class="container">
                <div class="row align-items-center">
                  <div class="col">
                    <h1 class="m-0 text-dark">Employee Leave Deductions</h1>
                  </div>
                </div>
              </div>
            </header>
    ...
    <section class="content ml-4 mr-4">
        ...
        <div class="modal fade" id="form_modal" tabindex="-1" role="dialog" aria-labelledby="modalLabel" aria-hidden="true">
          ...
          <table id="leave" class="table table-bordered table-striped"
			...
			<thead>
				<tr class="tr-class-1 text-center bg-secondary">
					<th class="table-plus datatable" data-sortable="true">EMPLOYEE NAME</th>
                    <th data-sortable="true">DEDUCTION AMOUNT</th>
			...
			<tbody>
                <?php
                    require 'conn.php';
                    $query = mysqli_query($conn, "SELECT * FROM `data`") or die($mysqli->error);
                    while($fetch = mysqli_fetch_array($query)){
                  ?>
                <tr class="tr-class-2 text-center">
                    <td><?php echo $fetch['name']?></td>
                    <td><?php echo number_format($fetch['leave_deduction'],2)?></td>
                    <td class="d-flex justify-content-center">
										
									<button class="btn btn-primary editbtn" data-toggle="modal" data-target="#update_modal<?php echo $fetch['id']?>">
    <i class="fas fa-pencil-alt"></i> 
</button>


										<!-- Update Salary Modal -->
										<div class="modal fade" id="update_modal<?php echo $fetch['id'] ?>" tabindex="-1" role="dialog" aria-labelledby="updateModalLabel" aria-hidden="true">
                                          <div class="modal-dialog modal-dialog-centered">
                                            <div class="modal-content">
                                              <form method="POST" action="update_leave.php">
                                                <input type="hidden" name="id" value="<?php echo $fetch['id'] ?>"/>
                                                <div class="modal-header bg-success text-white">
                                                  <h5 class="modal-title" id="updateModalLabel">Update Leave Deduction Amount</h5>
                                                  <button type="button" class="btn-close" data-dismiss="modal" aria-label="Close"></button>
                                                </div>
                                                <div class="modal-body">
                                                  <div class="mb-3">
                                                    <label for="deduction-amount" class="form-label">Deduction Amount</label>
                                                    <input type="text" name="leave_deduction" value="<?php echo $fetch['leave_deduction'] ?>" class="form-control" id="deduction-amount" required>
                                                  </div>
                                                </div>
                                                <div class="modal-footer">
                                                  <button type="button" class="btn btn-danger" data-dismiss="modal"><i class="bi bi-x"></i> Close</button>
                                                  <button type="submit" name="update" class="btn btn-warning"><i class="bi bi-pencil"></i> Update</button>
                                                </div>
                                              </form>
                                            </div>
                                          </div>
